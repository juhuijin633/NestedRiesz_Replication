...
class DiD_DGP:
    def __init__(self, dim_X=3, dim_Z = 3, beta1 = 1, beta2=1.5, c_1=1.0, delta_1 = 1 , delta_2 =1 , delta_3=1 , alpha_1 =1, gamma_1 = None, gamma_2= None, g  = None, gamma_10 = 0,gamma_11=0 , y_coef_1 = 0):
        self.dim_X = dim_X
        self.dim_Z = dim_Z
        self.beta1 = beta1
        self.beta2 = beta2
    ...
    def generate(self, n, seed=None):
        ...
        Y1 = self.beta1 * (X11 > 0).float() + self.delta_1 * X11 + Z @ self.gamma_2 + torch.randn(n) 
        ...
        X21 = X2[:, 0]
        Y2 = Y1 + self.c_1 * D.squeeze() + self.beta2 * (X21 > 0).float() + self.delta_3* D.squeeze() * X21 + torch.randn(n) +  (Y1>0).float()    * self.y_coef_1 
        return {
            "X1": X1,
            "X2": X2,
            "Y1": Y1,
    ...
    def simulate_att_population(self, n=int(5e6), batch_size=int(5e5), seed=None):
        ...
        def run_batch(m):
            ...
            Y1 = self.beta1 * (X11 > 0).float() + self.delta_1 * X11 + Z @ self.gamma_2 + eps_y1
            ...
            # === Potential outcomes
            Y2_0 = Y1 + self.beta2 * (X21_0 > 0).float()  + (Y1>0).float()    * self.y_coef_1 
            Y2_1 = Y1 + self.c_1 + self.beta2 * (X21_1 > 0).float() + self.delta_3 * 1.0 * X21_1 + (Y1>0).float()    * self.y_coef_1 
        ...
        # Run full batches
        for _ in tqdm(range(num_full), desc = "Running diferent batches to simualte the true ATT", leave= False):
...
folder = "results_non_lin_Y_final"
lasso_cv_settings = {
    'b_degree' : 1,
    'cv_folds' : folds,
    'random_state' : 42
...
for N in tqdm(Ns):
    for model_name, prop_func in propensity_models.items():
        ...
        for t in tqdm(range(tmax), desc=f"N:{N}, Model: {model_name}", leave=True):
            torch.manual_seed(t)
            X1_sub = X1[t*N:(t+1)*N, :]
            X2_sub = X2[t*N:(t+1)*N, :]
            ...
            pred_theta[t,4], pred_sig[t,4],*_ = estimateDynamicRiesz( Y1_sub, Y2_sub, D_sub, Z_sub, X1_sub, X2_sub, folds,
                                                                            method_a = "Net", lasso_a_settings = lasso_a_settings,
                                                                                method_f = "Net", lasso_f_settings = lasso_f_settings,
                                                                                seed = t)
