...
from utils.dgp import DiD_DGP, DiD_DGP_v2
...
for N in tqdm(Ns):
    for model_name, prop_func in propensity_models.items():
        ...
        dgp = DiD_DGP_v2(dim_X=dimX, dim_Z=dimZ, 
                        alpha_1 = 1, 
        ...
        ATT_calculations = dgp.simulate_att_population(n=int(5e8), batch_size=int(5e5))
        torch.save(ATT_calculations, f"{folder}/{file_suffix}_ATT_calculations_v2.pt")
        ...
        torch.save(pred_theta, f"{folder}/{file_suffix}_pred_theta_OLSv2.pt")
        torch.save(pred_sig, f"{folder}/{file_suffix}_pred_sig_OLSv2.pt")
