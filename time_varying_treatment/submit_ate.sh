...
#SBATCH --mem=16G
#SBATCH --array=0-5999
#SBATCH -o logs/job_ate_%a.out
#SBATCH -e logs/job_ate_%a.err
...
if [ $DGP_IDX -eq 0 ]; then
    $PYTHON run_sim_ate.py $ITER $N_VAL truncated_logistic
elif [ $DGP_IDX -eq 1 ]; then
    $PYTHON run_sim_nonlinear_ate.py $ITER $N_VAL truncated_adv
elif [ $DGP_IDX -eq 2 ]; then
    $PYTHON run_sim_ate.py $ITER $N_VAL truncated_adv
else
    $PYTHON run_sim_ate.py $ITER $N_VAL logistic
fi
