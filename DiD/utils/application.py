...
class application_data():
    ...
    def get_data(self, pre_treatment_year, post_treatment_year, treatment_year = None, baseline_2001 = False):

        if treatment_year is None:
            treatment_year = post_treatment_year

        g_filter = (self.data["G"] == 0) | (self.data["G"] > post_treatment_year) | (self.data["G"] == treatment_year)
        g_filtered_data = self.data[g_filter]
        filtered_data = g_filtered_data[(g_filtered_data["year"] == pre_treatment_year) | (g_filtered_data["year"] == post_treatment_year)]
        ...
        if baseline_2001:
            baselines = g_filtered_data[g_filtered_data["year"] == 2001].drop(columns = ["year"]+ remove_columns).reset_index(drop = True).add_prefix("2001_")
