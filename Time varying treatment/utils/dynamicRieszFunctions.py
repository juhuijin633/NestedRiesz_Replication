...
def estimateDynamicRiesz_all(Y, X, D, d, X_index, folds,
    ...
    fold_results = torch.zeros(Y.shape[0],9)
    RR1 = torch.zeros(Y.shape[0],9)
    RR2 = torch.zeros(Y.shape[0],9)
    f1 = torch.zeros(Y.shape[0],9)
    f2 = torch.zeros(Y.shape[0],9)
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(Y)):
        ...
        if estimate_rf_a & estimate_lasso_f:
        ...
        if estimate_net_a & estimate_lasso_f:
        ...
        if estimate_lasso_a & estimate_rf_f:
            fold_results[test_index,3:4], RR1[test_index,3:4], RR2[test_index,3:4], f1[test_index,3:4], f2[test_index,3:4] = thetahat(trainer.learners_f_RF, trainer.learners_a_LASSO, Y_test, X_test, D_test, d_test, X_index)   
        if estimate_rf_a & estimate_rf_f:
            fold_results[test_index,4:5], RR1[test_index,4:5], RR2[test_index,4:5], f1[test_index,4:5], f2[test_index,4:5] = thetahat(trainer.learners_f_RF, trainer.learners_a_RF, Y_test, X_test, D_test, d_test, X_index)   
        if estimate_net_a & estimate_rf_f:
            fold_results[test_index,5:6], RR1[test_index,5:6], RR2[test_index,5:6], f1[test_index,5:6], f2[test_index,5:6] = thetahat(trainer.learners_f_RF, trainer.learners_a_Net, Y_test, X_test, D_test, d_test, X_index)   
        if estimate_lasso_a & estimate_net_f:
            fold_results[test_index,6:7], RR1[test_index,6:7], RR2[test_index,6:7], f1[test_index,6:7], f2[test_index,6:7] = thetahat(trainer.learners_f_Net, trainer.learners_a_LASSO, Y_test, X_test, D_test, d_test, X_index)   
        if estimate_rf_a & estimate_net_f:
            fold_results[test_index,7:8], RR1[test_index,7:8], RR2[test_index,7:8], f1[test_index,7:8], f2[test_index,7:8] = thetahat(trainer.learners_f_Net, trainer.learners_a_RF, Y_test, X_test, D_test, d_test, X_index)   
        if estimate_net_a & estimate_net_f:
            fold_results[test_index,8:9], RR1[test_index,8:9], RR2[test_index,8:9], f1[test_index,8:9], f2[test_index,8:9] = thetahat(trainer.learners_f_Net, trainer.learners_a_Net, Y_test, X_test, D_test, d_test, X_index)   
