...
class Learner_f_LASSO_cv:

    """
    This class estimates the f_t function using LASSO in python with sklearn
    """
    
    def __init__(self, lasso_cv_settings = lasso_cv_settings_global):
        """
        Parameters
        ----------
        """
        
        self.b_degree = lasso_cv_settings['b_degree']
        self.cv_folds = lasso_cv_settings['cv_folds']
        self.random_state = lasso_cv_settings['random_state']  
    ...
    def fit(self, Y, X, D):
        ...
        b_func_class.set_standardization(X,D)
        self.b_func = b_func_class.b_poly
        XD = b_func_class.b_poly(X,D)

        # Fit LASSO with cross-validation
        lasso_cv = LassoCV(cv=self.cv_folds, random_state=self.random_state)  
        lasso_cv.fit(XD.numpy(), Y.numpy().ravel())

        self.rho_hat = torch.tensor(lasso_cv.coef_).reshape(-1,1).t()
